...
/**
 * REST controller for managing {@link com.mycompany.myapp.domain.CustomerDetails}.
 */
@RestController
@RequestMapping("/api")
public class CustomerDetailsResource {
    ...
    public ResponseEntity<CustomerDetails> createCustomerDetails(@Valid @RequestBody CustomerDetails customerDetails) throws URISyntaxException {
        ...
        if (Objects.isNull(customerDetails.getUser())) {
    ...
    public ResponseEntity<List<CustomerDetails>> getAllCustomerDetails(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("shoppingcart-is-null".equals(filter)) {
            log.debug("REST request to get all CustomerDetailss where shoppingCart is null");
            return new ResponseEntity<>(customerDetailsService.findAllWhereShoppingCartIsNull(),
                    HttpStatus.OK);
        }
