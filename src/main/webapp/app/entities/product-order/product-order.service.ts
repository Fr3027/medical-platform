...
export class ProductOrderService {
  ...
  protected convertDateFromClient(productOrder: IProductOrder): IProductOrder {
    const copy: IProductOrder = Object.assign({}, productOrder, {
      created: productOrder.created && productOrder.created.isValid() ? productOrder.created.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((productOrder: IProductOrder) => {
        productOrder.created = productOrder.created ? moment(productOrder.created) : undefined;
      });
    }
    return res;
  }
